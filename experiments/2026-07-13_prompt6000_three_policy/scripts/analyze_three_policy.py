#!/usr/bin/env python3
"""Request-paired analysis for the prompt-6000 three-policy experiment."""
...
def plot_paired_delta(paired, output):
    ...
    axis.set_title("Prompt 6000: request-paired E2E TTFT delta CDF", fontsize=16, pad=14)
...
def plot_arrival_bins(paired, output):
    ...
    grouped = paired.assign(time_bin=pd.cut(paired.send_time_s, bins=bins, labels=labels,
                                            include_lowest=True)).groupby("time_bin", observed=True)
    fig, axis = plt.subplots(figsize=(13.5, 6.8))
    for name in RUNS:
        short = SHORT[name]
        means = grouped[f"{short}_e2e_ttft_ms"].mean().reindex(labels)
        axis.plot(labels, means, marker="o", linewidth=2.4, color=COLORS[name], label=name)
    axis.set_xlabel("request send-time bin (s)", fontsize=11.5)
    axis.set_ylabel("mean E2E TTFT (ms)", fontsize=11.5)
    axis.set_title("Prompt 6000: E2E TTFT growth over the arrival window", fontsize=16, pad=14)
    axis.grid(True, color="#d9d2c8", linewidth=0.8)
    axis.set_axisbelow(True)
    axis.spines[["top", "right"]].set_visible(False)
...
def plot_home_gpu(paired, output):
    ...
    axis.set_title("Prompt 6000: mean E2E TTFT by original home cell", fontsize=16, pad=14)
