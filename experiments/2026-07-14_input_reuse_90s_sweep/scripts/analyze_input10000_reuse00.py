#!/usr/bin/env python3
"""Analyze the input-10000, zero-reuse, 90-second three-policy result."""
...
CASE = os.environ.get("ANALYSIS_CASE", "input10000_reuse00")
REUSE_LABELS = {
    "input10000_reuse00": "0%",
    "input10000_reuse025": "25%",
    "input10000_reuse05": "50%",
}
if CASE not in REUSE_LABELS:
    raise ValueError(f"Unsupported complete input-10000 case: {CASE}")
CASE_TITLE = f"Input 10000 / reuse {REUSE_LABELS[CASE]} / 90s"
RESULT_DIR = EXPERIMENT_DIR / "results" / CASE
FIGURE_DIR = EXPERIMENT_DIR / "figures" / CASE
ANALYSIS_DIR = EXPERIMENT_DIR / "analysis" / CASE
POLICIES = [
...
def plot_cdf(runs):
    ...
    subtitle = ("B and C overlap exactly because no KV is reusable"
                if CASE == "input10000_reuse00"
                else "All, redirected, and non-redirected requests")
...
def plot_arrival_bins(runs):
    ...
    labels = [f"{left}-{left + 10}" for left in bins[:-1]]
    fig, axis = plt.subplots(figsize=(13.5, 7.0))
