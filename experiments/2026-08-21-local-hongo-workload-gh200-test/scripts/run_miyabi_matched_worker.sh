...
cd "$ROOT"
mkdir -p "$EXP/results_miyabi_matched" "$EXP/logs_miyabi_matched"

# The checked-in Chakra protobuf was generated by protoc 7.35, while this
# host currently has the protobuf 6.33 runtime. The schema is compatible for
# this workload; bypass only the generated/runtime version guard.
export TEMPORARILY_DISABLE_PROTOBUF_VERSION_CHECK=true

run_one() {
  local number="$1" name="$2" policy="$3"
  local run_name="peak_${LEVEL}x_repeat600_seed1_miyabi_matched_${number}_${name}"
  local output="$EXP/results_miyabi_matched/$run_name"
  local log="$EXP/logs_miyabi_matched/$run_name.log"
  ...
  python3 -m serving \
    --cluster-config "$EXP/configs/gh200_hongo_8gpu_miyabi_matched.json" \
    --dataset "$EXP/workloads/hongo_peak_${LEVEL}x_repeat600_seed1.jsonl" \
