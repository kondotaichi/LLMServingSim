...
cd "$ROOT"
mkdir -p "$EXP/results_cloud_aligned" "$EXP/logs_cloud_trace_remake"
export TEMPORARILY_DISABLE_PROTOBUF_VERSION_CHECK=true

run_one() {
  local number="$1" name="$2" policy="$3"
  local run_name="peak_${LEVEL}x_repeat600_seed1_cloud_aligned_${number}_${name}"
  local output="$EXP/results_cloud_aligned/$run_name"
  local log="$EXP/logs_cloud_trace_remake/$run_name.log"
  ...
  python3 -m serving \
    --cluster-config "$EXP/configs/gh200_hongo_8gpu_cloud_aligned.json" \
    --dataset "$EXP/workloads_cloud_aligned/hongo_peak_${LEVEL}x_repeat600_seed1.jsonl" \
