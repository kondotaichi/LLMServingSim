...
ENVIRONMENTS = ("cloud", "apn")
LABELS = {
    "cloud": "Cloud (Miyabi-like)",
    "apn": "AI-RAN (APN, Active TCP 20%)",
}
METHOD_COLORS = {
    "cloud": "#2f5f9f",
    "apn": "#31866f",
}
LINE_STYLES = {"cloud": "--", "apn": "-"}
COMPONENTS = (
...
def plot_cdf(pp, peak, runs):
    ...
    axes[1].set_title("Router capacity-wait distribution", fontsize=16, pad=14)
    for axis in axes:
...
def plot_peak_sweep(summary, pp):
    ...
    for environment in ENVIRONMENTS:
        ...
        for metric, suffix, alpha in (
            ...
            ("p95_ttft_ms", "p95", 0.72),
            ("p99_ttft_ms", "p99", 0.45),
        ):
            axes[0].plot(data["peak_nx"], data[metric], color=METHOD_COLORS[environment],
                         linestyle=LINE_STYLES[environment], linewidth=2.4, alpha=alpha,
                         marker="o", markersize=4,
                         label=f"{LABELS[environment]} {suffix}")
    apn = subset[subset["environment"] == "apn"].sort_values("peak_nx")
    capacity_axis = axes[1].twinx()
    wait_line = axes[1].plot(
        apn["peak_nx"], apn["mean_router_capacity_wait_ms"],
        color="#c74b3a", linewidth=2.5, marker="o", label="Mean capacity wait",
    )
    request_line = capacity_axis.plot(
        apn["peak_nx"], apn["initially_inadmissible_requests"],
        color="#865bd6", linewidth=2.5, marker="s",
        label="Initially inadmissible requests",
    )
    axes[0].set_ylabel("E2E TTFT (ms)", fontsize=12)
    axes[0].set_yscale("log")
    axes[0].set_title("TTFT across offered load", fontsize=16, pad=14)
    axes[1].set_ylabel("Mean capacity wait (ms)", fontsize=12, color="#c74b3a")
    capacity_axis.set_ylabel("Initially inadmissible requests out of 600",
                             fontsize=12, color="#865bd6")
    axes[1].set_title("AI-RAN capacity pressure", fontsize=16, pad=14)
    for axis in axes:
    ...
    axes[1].legend(wait_line + request_line,
                   [line.get_label() for line in wait_line + request_line],
                   frameon=False, fontsize=8.5, loc="upper left")
    axes[1].tick_params(axis="y", colors="#c74b3a")
    capacity_axis.tick_params(axis="y", colors="#865bd6")
    capacity_axis.spines["top"].set_visible(False)
    fig.suptitle(f"PP={pp}: Active TCP 20% peak sweep — NEAREST_KV", fontsize=17)
    fig.tight_layout()
    fig.savefig(FIGURE_ROOT / f"pp{pp}_peak_sweep.png", dpi=180,
                bbox_inches="tight", facecolor=BACKGROUND)
    plt.close(fig)


def paired_comparison(summary):
    cloud = summary[summary["environment"] == "cloud"].set_index(["pp", "peak_nx"])
    apn = summary[summary["environment"] == "apn"].set_index(["pp", "peak_nx"])
    rows = []
    for index in cloud.index:
        c = cloud.loc[index]
        a = apn.loc[index]
        rows.append({
            "pp": index[0],
            "peak_nx": index[1],
            "cloud_mean_ttft_ms": c["mean_ttft_ms"],
            "apn_mean_ttft_ms": a["mean_ttft_ms"],
            "apn_minus_cloud_mean_ttft_ms": a["mean_ttft_ms"] - c["mean_ttft_ms"],
            "apn_over_cloud_mean_ttft": a["mean_ttft_ms"] / c["mean_ttft_ms"],
            "cloud_p95_ttft_ms": c["p95_ttft_ms"],
            "apn_p95_ttft_ms": a["p95_ttft_ms"],
            "apn_mean_capacity_wait_ms": a["mean_router_capacity_wait_ms"],
            "apn_initially_inadmissible_requests": int(a["initially_inadmissible_requests"]),
            "apn_min_available_kv_gb": a["min_available_kv_gb"],
            "apn_electricity_yen_per_hour": a["estimated_electricity_yen_per_hour"],
            "miyabi_yen_per_hour": MIYABI_YEN_PER_HOUR,
            "apn_electricity_yen_per_1000_requests": a["estimated_electricity_yen_per_1000_requests"],
            "miyabi_yen_per_1000_requests": c["miyabi_yen_per_1000_requests"],
        })
    return pd.DataFrame(rows).sort_values(["pp", "peak_nx"])
...
def main():
    ...
    summary = pd.DataFrame(rows).sort_values(["pp", "peak_nx", "environment"])
    summary.to_csv(ANALYSIS_ROOT / "summary.csv", index=False)
    paired_comparison(summary).to_csv(ANALYSIS_ROOT / "paired_comparison.csv", index=False)
    for pp in (1, 2):
        plot_peak_sweep(summary, pp)
