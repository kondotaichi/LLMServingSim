...
# Method A ("scheduler-hide"): parallelizes KV migration transfer with the
# target scheduler's queue wait instead of serializing them, layered on top
# of the *unchanged* PP2 baseline routing policy
# (NEAREST_CAPACITY_MULTI_PRESSURE_KV_RESERVE). No redirect-decision logic
# changes here -- see run_pp2_speculative_parallel.sh for Method B, which
# additionally needs the formula policy + local-wait recalibration.
# See .claude/plans/elegant-dazzling-lobster.md for the full design.
...
if [[ "$(uname -s)" != "Linux" ]]; then
  ...
  echo "Dispatching five PP2-scheduler-hide simulations into ${SIM_CONTAINER}..."
  exec docker exec \
    ...
    "${SIM_CONTAINER}" \
    bash -lc "cd /app/LLMServingSim && ./${EXPERIMENT_REL}/run_pp2_scheduler_hide_parallel.sh"
fi
...
run_workload() {
  ...
  if env TEMPORARILY_DISABLE_PROTOBUF_VERSION_CHECK=true \
    PYTHONUNBUFFERED=1 "${PYTHON_BIN}" -u -m serving \
      ...
      --run-id "pp2-scheduler-hide-five-server-v2-${workload}" \
