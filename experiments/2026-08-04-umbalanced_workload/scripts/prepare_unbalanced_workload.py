#!/usr/bin/env python3
"""Build a *spatially imbalanced* campus workload with 12 fixed physical GPUs.

Forked from the balanced Hongo generator. The only structural change is user
placement: instead of a uniform spread over the campus, users are concentrated
into gaussian hotspots around a subset of PP2 groups. This drives one PP2
instance into saturation so the router is forced to redirect to cold neighbors,
which is the regime where carrying the KV cache across a redirect actually pays
off (and where the balanced workload showed 0 redirects under PP2).
...
- users are drawn from gaussian clusters (see HOTSPOT_* below), targeting a
  strongly skewed per-PP-group request share (top group ~65%)
"""
...
# --- Spatial imbalance (gaussian hotspots) ---
# Target request share per PP2 group (index 0..NUM_PP_GROUPS-1). The realized
# per-group share after nearest-GPU assignment is printed at generation time so
# these can be tuned; values need not sum to exactly 1 (they are normalized).
# Strong skew: the top group carries ~65% (max/mean per group ~4x).
TARGET_GROUP_SHARES = (0.72, 0.10, 0.06, 0.05, 0.04, 0.03)
# Std-dev (meters) of the gaussian cluster around each group centroid. Smaller
# = tighter concentration = stronger pressure on that group's instance.
...
def group_centroids(positions: list[dict]) -> list[tuple[float, float]]:
    """Centroid (x, y) of each PP2 group's PP_GROUP_SIZE physical GPUs."""
    centroids = []
    for group_id in range(NUM_PP_GROUPS):
        gpus = [positions[gid] for gid in range(group_id * PP_GROUP_SIZE, (group_id + 1) * PP_GROUP_SIZE)]
        cx = sum(p["gpu_x_m"] for p in gpus) / len(gpus)
        cy = sum(p["gpu_y_m"] for p in gpus) / len(gpus)
        centroids.append((cx, cy))
    return centroids


def sample_hotspot_xy(rng: random.Random, centroid: tuple[float, float]) -> tuple[float, float]:
    """Gaussian sample around a group centroid, clipped to the campus square."""
    cx, cy = centroid
    x = min(max(rng.gauss(cx, HOTSPOT_SIGMA_M), 0.0), SIDE_M)
    y = min(max(rng.gauss(cy, HOTSPOT_SIGMA_M), 0.0), SIDE_M)
    return x, y


def create_placements(positions: list[dict]) -> list[dict]:
    rng = random.Random(PLACEMENT_SEED)
    centroids = group_centroids(positions)
    weights = [max(0.0, w) for w in TARGET_GROUP_SHARES]
    if len(weights) != NUM_PP_GROUPS:
        raise ValueError(
            f"TARGET_GROUP_SHARES has {len(weights)} entries, expected {NUM_PP_GROUPS}."
        )
    users = []
    for user_id in range(CAMPUS_POPULATION):
        # Draw the user's home PP2 group from the target categorical, then place
        # them with a gaussian jitter around that group's centroid so the nearest
        # physical GPU (and thus PP group) is that group with high probability.
        home_group = rng.choices(range(NUM_PP_GROUPS), weights=weights, k=1)[0]
        xy = sample_hotspot_xy(rng, centroids[home_group])
...
def build_workload(
    ...
    group_counts = Counter(gid // PP_GROUP_SIZE for gid in (r["gpu_id"] for r in rows))
    group_share = {g: group_counts.get(g, 0) / n for g in range(NUM_PP_GROUPS)}
    top_share = max(group_share.values())
    print(
        f"  [{label}] per-PP-group share: "
        + " ".join(f"g{g}={group_share[g]*100:.0f}%" for g in range(NUM_PP_GROUPS))
        + f"  (top={top_share*100:.0f}%, max/mean={top_share*NUM_PP_GROUPS:.1f}x)"
    )
    manifest = {
