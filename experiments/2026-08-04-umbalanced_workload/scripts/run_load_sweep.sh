...
# Canonical ShareGPT-derived session content (input/output token ids). We only
# reuse the per-session text; user placement is re-drawn as hotspots here.
CONTENT_SOURCE="${CONTENT_SOURCE:-$REPO_ROOT/experiments/2026-08-01_hongo_workload/workloads/hongo_peak_2x_seed1.jsonl}"
...
ensure_pp2_workload() {
  local load_level="$1"
  local base_path="$EXP_ROOT/workloads/unbal_${load_level}_seed${SEED}.jsonl"
  local pp2_path="$EXP_ROOT/workloads/unbal_${load_level}_seed${SEED}_pp2.jsonl"
  if [[ -f "$pp2_path" ]]; then
    return 0
  fi
  if [[ ! -f "$base_path" ]]; then
...
module_path = Path("$EXP_ROOT/scripts/prepare_unbalanced_workload.py")
spec = importlib.util.spec_from_file_location("prepare_unbalanced_workload", module_path)
...
prepare_workloads() {
  ...
  for level in $LEVELS; do
    ...
    base_path="$EXP_ROOT/workloads/unbal_${load_level}_seed${SEED}.jsonl"
    pp2_path="$EXP_ROOT/workloads/unbal_${load_level}_seed${SEED}_pp2.jsonl"
  ...
  if ((${#deduped[@]} > 0)); then
    python3 "$EXP_ROOT/scripts/prepare_unbalanced_workload.py" \
...
run_level() {
  ...
  (
    ...
    bash scripts/run_probe_6arm.sh
    ...
    python3 scripts/plot_ttft_breakdown.py \
      ...
      --title "Unbalanced ${level} (${NUM_REQS} req, seed${SEED}): five-arm TTFT breakdown" \
      --cdf-title "Unbalanced ${level} (${NUM_REQS} req, seed${SEED}): TTFT CDF"
  )
}
