#!/usr/bin/env python3
"""Plot the prompt-6000, 90-second comparison across three policies."""
...
def plot_breakdown(rows_by_method, output):
    ...
    axis.set_title("Prompt 6000 / 90s: mean E2E TTFT breakdown\nThree nearest-GPU policies",
                   fontsize=17, pad=18)
...
def write_handoff_breakdown(rows_by_method, image_output, csv_output):
    ...
    axis.set_xlabel("mean E2E TTFT components (ms)", fontsize=12)
    axis.set_title("Prompt 6000 / 90s: E2E TTFT breakdown\nAll, redirected, and non-redirected requests",
                   fontsize=17, pad=18)
    axis.grid(axis="x", color="#d9d2c8", linewidth=0.8)
    axis.set_axisbelow(True)
    axis.spines[["top", "right", "left"]].set_visible(False)
...
def plot_handoff_cdf(rows_by_method, output):
    ...
    axis.set_title("Prompt 6000 / 90s: E2E TTFT CDF\nAll, redirected, and non-redirected requests",
                   fontsize=17, pad=16)
...
def plot_gpu_load(rows_by_method, output):
    ...
    axis.set_title("Prompt 6000 / 90s: per-GPU processed request count", fontsize=17, pad=14)
