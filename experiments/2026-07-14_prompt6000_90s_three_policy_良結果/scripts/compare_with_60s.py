#!/usr/bin/env python3
"""Compare the prompt-6000 90-second experiment with the 60-second run."""
...
def build_comparison(old_summary, new_summary):
    comparison = {"baseline_window_s": 60, "current_window_s": 90, "policies": {}}
...
def plot_comparison(comparison, output):
    ...
    mean_90 = [comparison["policies"][policy]["mean_ttft_90s_ms"] for policy in POLICIES]
    p95_60 = [comparison["policies"][policy]["p95_ttft_60s_ms"] for policy in POLICIES]
    p95_90 = [comparison["policies"][policy]["p95_ttft_90s_ms"] for policy in POLICIES]
    redirects_60 = [comparison["policies"][policy]["redirects_60s"] for policy in POLICIES]
    redirects_90 = [comparison["policies"][policy]["redirects_90s"] for policy in POLICIES]

    panels = [
        (axes[0], mean_60, mean_90, "mean E2E TTFT (ms)"),
        (axes[1], p95_60, p95_90, "p95 E2E TTFT (ms)"),
        (axes[2], redirects_60, redirects_90, "redirected requests"),
    ]
    short_labels = ["NEAREST_KV", "MIGRATE", "MIGRATE_KV"]
    for axis, values_60, values_90, title in panels:
        axis.bar(x - width / 2, values_60, width, color="#9b9892", label="60s")
        axis.bar(x + width / 2, values_90, width, color="#3a6aaa", label="90s")
    ...
    fig.suptitle("Prompt 6000: 60s vs 90s arrival window", fontsize=18, y=1.02)
    fig.tight_layout()
